 ...
 */
export function ModularLayout({ vendor, template, products, happyBanner }: VendorLayoutProps) {
  const Skeleton = getSkeleton(template.skeleton ?? "classic");
  return <Skeleton vendor={vendor} template={template} products={products} happyBanner={happyBanner} />;
}
